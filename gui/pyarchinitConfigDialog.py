...
class pyArchInitDialog_Config(QDialog, MAIN_DIALOG_CLASS):
    ...
    def __init__(self, parent=None, db=None):
        ...
        self.pushButtonPostgis.clicked.connect(self.setPathPostgis)
        self.pbnSaveEnvironPath_P.clicked.connect(self.setEnvironPath_P)
        ...
        self.postgis_bin = s.value('pyArchInit/postgisBinPath', None, type=str)
        if self.postgis_bin:
            self.lineEditPostgis.setText(self.postgis_bin)

        if Pyarchinit_OS_Utility.checkPostgisInstallation():
            self.pushButtonPostgis.setEnabled(False)
            self.pbnSaveEnvironPath_P.setEnabled(False)
            self.lineEditPostgis.setEnabled(False)
    ...
    def setPathPostgis(self):
        s = QgsSettings()
        self.postgis_bin = QFileDialog.getExistingDirectory(
            self,
            "Set path directory",
            self.HOME,
            QFileDialog.ShowDirsOnly
        ...
        if self.postgis_bin:
            self.lineEditPostgis.setText(self.postgis_bin)
            s.setValue('pyArchInit/postgisBinPath', self.postgis_bin)
    ...
    def setEnvironPath_P(self):
        os.environ['PATH'] += os.pathsep + os.path.normpath(self.postgis_bin)
        QMessageBox.warning(self, "Set Environmental Variable", "The path has been set successful", QMessageBox.Ok) 
